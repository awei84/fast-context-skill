...
- **不做本地 `semble` 兜底。** 不只是为了保「纯 Node、零依赖」，更本质的理由是:**fallback 不是免费的鲁棒性,它会改变 agent 的证据质量。** 本工具的消费者是 agent,远端失败时 agent 本就能退回 `rg`、读目录、读文件,按错误和已知关键词自己探索——这条路径朴素但证据链清楚。低置信兜底反而有害:它返回的是 chunk 相似度而非经代码工具验证的结论,query 模糊时容易给「看起来相关」的近邻文件,agent 会因此过早锚定、少做必要核验。所以本项目不追求「任何情况下都有结果」,而追求「结果必须值得 agent 继续相信和核验」。远端失败时返回明确错误,比给低置信 fallback 更安全。
- 不走 Python 依赖路线。
- 不替代 MCP 包;本项目只提供独立的 CLI + Skill 形态。
- 不做多模型 fallback / 重试退避:交互式工具失败时人会重看,限流时自动重试反添乱,且本项目认定主模型 `WS_MODEL` 最优,切备用 = 主动降级(逻辑同上一条)。保留单模型配置和已有 HTTP 重试即可。
